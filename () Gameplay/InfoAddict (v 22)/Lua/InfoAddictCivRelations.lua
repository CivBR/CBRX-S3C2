  ...
  for thisPid = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
    if( hasMetCiv(thisPid) and Players[thisPid]:IsAlive()) then
      ...
      repeat
		...
		logger:trace("itemType: " .. tostring(itemType) .. ",  duration: " .. tostring(duration) .. ",  finalTurn: " .. tostring(finalTurn) ..
		             ",  data1: " .. tostring(data1) .. ",  data2: " .. tostring(data2) .. ",  data3: " .. tostring(data3) ..
  ...
  for thisPid = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
    if (tooltipData["hostile"][thisPid] ~= nil) then
      ...
      if (tooltipData["hostile"][thisPid] ~= "") then
        str = str .. "[NEWLINE]" .. hostile_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_HOSTILE") ..
              ":[ENDCOLOR] " .. tooltipData["hostile"][thisPid];
      end;

      -- Guarded?
      if (tooltipData["guarded"][thisPid] ~= "") then
        str = str .. "[NEWLINE]" .. guarded_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_GUARDED") ..
              ":[ENDCOLOR] " .. tooltipData["guarded"][thisPid];
      end;

      -- afraid?
      if (tooltipData["afraid"][thisPid] ~= "") then
        str = str .. "[NEWLINE]" .. afraid_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_AFRAID") ..
              ":[ENDCOLOR] " .. tooltipData["afraid"][thisPid];
      end;

      -- friendly?
      if (tooltipData["friendly"][thisPid] ~= "") then
        str = str .. "[NEWLINE]" .. friendly_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_FRIENDLY") ..
              ":[ENDCOLOR] " .. tooltipData["friendly"][thisPid];
      end;

      -- Neutral?
      if (tooltipData["neutral"][thisPid] ~= "") then
        str = str .. "[NEWLINE]" .. neutral_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_NEUTRAL") ..
