...
function UpdateNewData(playerID, szTag, isOverride)
	...
	if (not player:IsTurnActive()) and (not isOverride) then
		return;
	end
...
local maxCivs = GameDefines.MAX_PLAYERS
local civs = {}
local civsCount = 1
for iPlayer = 0, maxCivs do
	if Players[iPlayer] and Players[iPlayer]:IsEverAlive() and (not Players[iPlayer]:IsMinorCiv()) and (not Players[iPlayer]:IsBarbarian()) then
		civs[civsCount] = iPlayer
		civsCount = civsCount + 1
	end
end		
function InputHandler( uiMsg, wParam, lParam )
    ...
    -- Key Down Processing
    ----------------------------------------------------------------   
	if wParam == 0 then return end
	if uiMsg == KeyEvents.KeyDown then
		if (wParam == Keys.N) then
			-- for _,iPlayer in ipairs(civs) do
				-- print(_,iPlayer,Players[iPlayer]:GetName())
				currentPlayerPos = currentPlayerPos + 1
				-- if currentPlayer ~= iPlayer and Players[iPlayer]:IsEverAlive() and (not Players[iPlayer]:IsMinorCiv()) and (not Players[iPlayer]:IsBarbarian()) then
					currentPlayer = civs[currentPlayerPos]
					if currentPlayerPos > #civs then currentPlayerPos = 0 end
					-- break
				-- elseif currentPlayerPos == #civs then
					-- currentPlayer = 0
					-- currentPlayerPos = 1
					-- break
				-- end
			-- end
			UpdateNewData(currentPlayer,nil,true)
		end
		...
		if (wParam == Keys.B) then
			-- for _,iPlayer in ipairs(civs) do
				-- print(_,iPlayer,Players[iPlayer]:GetName())
				currentPlayerPos = currentPlayerPos - 1
				if currentPlayerPos < 0 then currentPlayerPos = #civs end
				-- if currentPlayer ~= iPlayer and Players[iPlayer]:IsEverAlive() and (not Players[iPlayer]:IsMinorCiv()) and (not Players[iPlayer]:IsBarbarian()) then
					currentPlayer = civs[currentPlayerPos]
					-- break
				-- elseif currentPlayerPos == #civs then
